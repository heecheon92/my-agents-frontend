...
- **Ask** — conversation list and transcript, server-owned messages, streamed answers, document-level sources with complete or partial read coverage, a verified answer process retained on the latest answer with a per-stage operational summary on each step, quiet model-authored process notes inside that disclosure, knowledge-base selection, and durable V1/V2 document clarification with ranked candidates, bounded filename refinement, and broad fallback.
- **Conversation attachments** — shown only when both the deployment and the account allow it. Stage a temporary file for this conversation, consent to the transfer before each upload, and send it with your question. The file is never added to a knowledge base and expires on its own; only the formats the served registry certifies come back as a downloadable edited file. Accepted formats, file counts, size ceilings, and retention all come from the backend.
