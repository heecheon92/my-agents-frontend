...
제품 채팅은 `/conversations/{id}/runs`를 사용합니다. `/assistant/chat`은 레거시/개발용이므로 제품 BFF proxy에서 차단합니다. 회원가입은 백엔드 계약을 그대로 따릅니다. 계정 생성은 안전한 user payload(`id`, `email`)를 반환하며, 사용자는 같은 자격 증명으로 로그인할 수 있습니다.
