...
- 지식/소스 워크플로: 개인/그룹 지식 공간 생성/목록, 권한 사용자 전용 시스템 프로젝트 지식 공간, 선택한 지식 공간으로 PDF/Markdown/plain-text와 `.xlsx`/`.pptx`/`.docx` 드래그 앤 드롭 또는 파일 선택 업로드, 텍스트 소스 추가, 검색 준비/처리 내역.
- 그룹: 생성/목록, 초대 수락 기반 멤버십, 계정 없는 초대 수신자의 nickname/password 가입, 공유 지식 게시 요청, 사용자 탐색 없는 역할 관리.
