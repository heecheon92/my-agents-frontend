...
- 지식/소스 워크플로: 개인/그룹 지식 공간 생성/목록/이름 변경/삭제, 권한 사용자 전용 시스템 프로젝트 지식 공간, 선택한 지식 공간으로 PDF/Markdown/plain-text와 `.xlsx`/`.pptx`/`.docx` 드래그 앤 드롭 또는 파일 선택 업로드, 텍스트 소스 추가, Markdown preview, 검색 준비/처리 내역.
- 그룹: 생성/목록, 초대 수락 기반 멤버십, 계정 없는 초대 수신자의 nickname/password 가입, 공유 지식 게시 요청 review/status, 사용자 탐색 없는 역할 관리. 공유 요청 생성은 Groups가 아니라 Knowledge의 지식 공간/문서 action에서 selector 기반으로 수행합니다.

Ask 화면은 `/conversations/{id}/runs`를 사용합니다. `/assistant/chat`은 레거시/개발용이므로 제품 BFF proxy에서 차단합니다. 권한 사용자가 구성한 시스템 프로젝트 지식은 Ask에 자동으로 추가될 수 있으며, 사용자별 memory나 개인/그룹 선택으로 끄는 출처가 아닙니다. 그룹 멤버십은 초대 수락 전용입니다. UI는 사용자 검색, 계정 존재 여부 확인, 직접 `user_id` 활성화를 암시하지 않아야 하며, group knowledge를 선택해도 conversation과 memory는 사용자별 private 범위로 남습니다. 계정이 없는 초대 수신자는 초대 링크에서 email을 다시 입력하지 않고 표시 이름과 비밀번호만 정하며, 이후 로그인은 표시 이름이 아니라 초대받은 email과 비밀번호로 합니다. 일반 회원가입은 hosted OpenAPI 계약을 그대로 따릅니다. 계정 생성은 `{ user, verification_email_sent }`를 반환하며, 사용자는 필요한 인증 흐름 이후 같은 자격 증명으로 로그인할 수 있습니다.
