...
| `/chat` | `components/ChatWorkspace.tsx` + `components/chat/*` | Anchor Ask journey. Uses conversations, messages, streamed answers, document-level sources and complete/partial read coverage near replies, a compact source selector, and a reached-stage agent process that accumulates live and remains compactly attached to the latest answer. |
