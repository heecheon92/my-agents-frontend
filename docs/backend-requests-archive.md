# Backend Requests archive

This is the preserved append-only history that previously served as the active
backend-request ledger. Its statuses and deployment observations are historical
snapshots and are **not current authority**.

Use [`backend-requests.md`](./backend-requests.md) for current backend gaps,
priority, and verification gates. Follow this archive only when investigating
the rationale or evidence behind a resolved, withdrawn, or superseded request.
...
## 2026-09-02 — raw reranker enum reaches user-facing trace copy

Status: proposed — frontend cannot fix this without violating the boundary.

Frontend need: agent-trace step descriptions are rendered verbatim, so any
implementation vocabulary the backend puts in them reaches the reader
unfiltered.

`RagAgentStage(id="evidence_judge")` builds its description by interpolating the
`RerankerMode` enum value directly into display prose:
...
Users see `cross_encoder 관련도 정렬을 적용했습니다.` — a raw settings enum in a
primary reading path, which `docs/korean-copy-guide.md` bans outright ("raw
enums, or bare identifiers"). It also discloses that two reranker modes exist
and which one this deployment runs, neither of which is user-facing information.

Requested backend contract: drop the interpolated identifier from the display
string. `관련도 정렬을 적용했습니다.` / `Applied relevance ordering.` says
everything a reader needs. The mode is already carried in
`evidence={"reranker": reranker}`, which is the right place for it — structured
evidence rather than prose.

If the distinction is genuinely user-visible, send a semantic key plus an
allowlisted parameter, per the 2026-08-26 operational-summary contract, rather
than interpolated prose. Do not localize the enum value itself.

Frontend impact and urgency: until 2026-09-02 this string was effectively
invisible. Nothing in `phaseForTraceStep` maps to `answerReady`, the last stage
of every successful run, so the old description gate
(`isCurrent || (terminal && stage is last)`) could never fire at terminal and no
description rendered once a run finished. That gate has now been fixed, and the
collapsed process row shows the newest step description while a run works — so
this string is about to appear on the primary answer surface of every retrieval
run rather than nowhere.

Note also that `tests/knowledge-copy.test.ts` only scans `localization/*.json`.
Backend-authored trace text bypasses `JARGON_RULINGS` entirely, so this class of
leak has no frontend guard by construction.
