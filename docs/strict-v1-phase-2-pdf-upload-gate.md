...
- Document UI uploads PDF/Markdown/plain-text/`.xlsx`/`.pptx`/`.docx` files into the selected knowledge base.
...
- Final browser evidence should upload a supported PDF, Markdown, plain-text, `.xlsx`, `.pptx`, or `.docx` file through the product UI, ingest it, and show source metadata/citations. If the launch gate intentionally uses the JSON text-document fallback instead, record that fallback and reason in `docs/public-demo-release-runbook.md` evidence bundle fields.
