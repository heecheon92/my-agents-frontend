...
6. Upload one or more supported PDF, Markdown, plain-text, `.xlsx`, `.pptx`, or `.docx` files through the in-card upload queue; each accepted guest upload counts toward the backend-owned 3-document cap.
...
- Document upload/create + ingest: uploaded PDF/Markdown/plain-text/`.xlsx`/`.pptx`/`.docx` preferred; JSON text-document fallback reason required if used
- Streamed chat:
- Citations:
- Redacted events:
- Refresh persistence:
