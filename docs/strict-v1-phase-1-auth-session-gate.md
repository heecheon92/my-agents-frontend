...
| Signup-disabled surfacing | Backend public-demo signup-disable support should return a safe non-2xx `{ detail }` body such as the planned/paired `MY_AGENTS_AUTH_SIGNUP_ENABLED=false` 403 path. The existing fetch client/AuthPanel path will render that string in the auth error panel without adding frontend-only fields. | Frontend-ready if backend detail is safe; backend owns the exact switch, status, and copy. |
