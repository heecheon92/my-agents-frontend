...
1. **Hosted upload contract evidence**: frontend uploaded-file support exists, but the hosted preview backend/OpenAPI used for the reviewer demo must expose `POST /knowledge-bases/{knowledge_base_id}/documents/upload` and its PDF/Markdown/plain-text/`.xlsx`/`.pptx` limits/error behavior.
...
4. Document proof: uploaded supported PDF/Markdown/plain-text/`.xlsx`/`.pptx` preferred; if using JSON text-document fallback, record the backend/infrastructure reason and limitation in the evidence bundle.
